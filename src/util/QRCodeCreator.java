...
public class QRCodeCreator {  
    ...
    private BufferedImage createCodeImage(QRCodeModel info){  
    ...
    public void createCodeImage(QRCodeModel info, OutputStream output){  
...
//              height = info.getBottomEnd()[1]+1;  
                ...
                //计算行数，目前计算字数在 12-16字之间，会少一行，所以linNum设为2，就是在计算的行数基础上多加一行，解决却一行掉字问题  
                int lineNum = 2;  
        ...
        try{  
            ImageIO.write(bm, info.getFormat().isEmpty() ? info.getFormat() : info.getFormat(), output);  
        }catch(Exception e){  
            e.printStackTrace();  
        }  
    }  
      
    public void createCodeImage(QRCodeModel info, File file){  
    ...
    public void createCodeImage(QRCodeModel info, String filePath){  
